...
function DeckForm({ getDecks }) {
  ...
  async function searchDecks(checkedState, deckName, e) {
    ...
    for (let color of deckColor) {
      try {
        let deckAxiosString = `https://deckmaster.herokuapp.com/decks?format=${deckLegality}&colors=${color}&name=${capitalizedName}`
    ...
    if (checkedState['multicolored'] === true) {
      ...
      try {
        let deckAxiosString = `https://deckmaster.herokuapp.com/decks?format=${deckLegality}&colors=${deckColor}&name=${capitalizedName}`
